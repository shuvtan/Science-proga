...
class CalcMesh:
    ...
    def snapshot(self, snap_number):
        ...
        writer.SetFileName("3povorot" + str(snap_number) + ".vtu")
        writer.Write()
...
def calc (n, angle): #ось - прямая, массив из 3 чисел [n1, n2, n3]
    ...
    for i in range(int(len(nodesCoord) / 3)):
        err2 = np.zeros(3)
        r = [nodes[0][i], nodes[1][i], nodes[2][i]]
        #print('r=',r)
        ...
        #print(modul_velocity)
        s = np.zeros(3)
        if err != [0, 0, 0]:
            s = np.cross(np.array(err), n)
...
for i in range(1, 100):
    nodes = mesh.nodes
    print("nodes1:", i, "-", nodes[0][12], nodes[1][12], nodes[2][12])
    print("nodes2:", i, "-", nodes[0][15], nodes[1][15], nodes[2][15])
    print(math.sqrt(((nodes[1][12]-nodes[1][15])*(nodes[1][12]-nodes[1][15])) + ((nodes[0][12]-nodes[0][15])*(nodes[0][12]-nodes[0][15])) + ((nodes[2][12]-nodes[2][15])*(nodes[2][12]-nodes[2][15]))))
    mesh.rotate(calc([0,0,1], i/50))
    mesh.snapshot(i)
