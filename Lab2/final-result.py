#Итоговый код рандомного вращения (подбрасывания) кубика в невесомости
...
class CalcMesh:
    ...
    #Функция поворота кубика, изменение координат и поля перемещения
    ...
    def snapshot(self, snap_number):
        ...
        writer.SetFileName("result-povorot" + str(snap_number) + ".vtu")
        writer.Write()
...
###################################################################################
#Функция вычисления перемещения точки кубика вокруг оси, заданной вектором n (n[0]x+n[1]y+n[2]z = 0), на угол angle, выбрав центр координат centr (centr[0],centr[1],centr[2])
def calc (n, angle, centr): #ось - прямая, массив из 3 чисел [n1, n2, n3], координаты точки начала системы координат (x,y,z) - массив из 3 чисел [x, y, z]
    v1 = []
    v2 = []
    v3 = []
    for i in range(int(len(nodesCoord) / 3)):
        err2 = np.zeros(3)
        r = np.array([nodes[0][i], nodes[1][i], nodes[2][i]]) - centr
...
mesh.snapshot(0)
for i in range(1, int(random.uniform(80,150))):
    nodes = mesh.nodes
    ...
    #Поворот кубика вокруг рандомной оси, с центром координат в рандомной точке на угол pi/6
    mesh.rotate(calc(np.array([random.uniform(0.00001,1), random.uniform(0,1), random.uniform(0,1)]), math.pi/6, np.array([random.uniform(0.00001,1), random.uniform(0,1), random.uniform(0,1)])))
    mesh.snapshot(i)
